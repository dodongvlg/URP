...
def main_work():
    ...
    with open(write_log.file_name, 'r') as f:
        ...
        last_line = lines[-1]
        if ("moving" in last_line):
            write_log.write_log(4, None, None, None, None, time_diff)

    zero_line = (search_string_in_file(write_log.file_name, 'end, plate'))[0][0]
    total_line = file_len(write_log.file_name)
    case_num = - (- (total_line - zero_line) / 3) + 1
    print(case_num)

    if (case_num >= 19):
        write_log.write_log(3, None, 4, None, None, time_diff)
        return
    ...
    write_log.write_log(1, speed, 4, catching_dir, pos, time_diff)
...
def input_work(speed, catching_dir, pos):
    ...
    write_log.write_log(7, speed, 4, catching_dir, pos, time_diff)
    give(catching_dir, pos)
    y.reset_home()
    y.right.open_gripper()
    write_log.write_log(2, None, None, None, None, time_diff)
...
if __name__ == '__main__':
    ...
    if os.path.isfile('random_test4.txt'):
        pass
    else:
        f = open('random_test4.txt', 'w')
